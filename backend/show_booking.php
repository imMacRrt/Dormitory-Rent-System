...
<script  type="text/javascript"> 
    $(document).ready(function(){
        ...
        $(".delbook").click(function(){
          var ind = $('.delbook').index(this);
          var id_val = $('.delbook').eq(ind).attr('id');
          if(confirm("ยืนยันการยกเลิกการจอง")){
            window.location.replace("cancel_booking.php?id="+id_val);
          }
        })
    });
</script>
...
<?php  
    ...
    $search = $_POST['search'];
    if($search!=''){
      $where_condition .= " AND ( a.bookid LIKE '%".$search."%' OR b.cusname LIKE '%".$search."%' )";
      }
?>
                ...
                <div class="pcoded-inner-content">
                  <div class="main-body">
                    <div class="page-wrapper">
                      <!-- Page-body start -->
                      <div class="page-body">
                        <form id = form_search method = "POST" action = "show_booking.php">
                          <div class="row">
                            <div class="col-sm-12">
                            ...
                            <table id="demo-foo-filtering" class="table table-striped">
                              ...
                              <tbody>
                                ...
                                <tr>
                                  <td align="right"><?php echo $row['bookid'];?> </td>                  
                                  ...
                                  <td><?php if($row['bookevidence'] !=''){ ?>
                                  <img src="upload/evidence/<?php echo $row['bookevidence'];?>" width="100px">
                                      <?php }else{ ?>
                                        <p>-</p>
                                  <?php }
                                  ?></td>
                                  <td><?php echo $bookstatus[$row['bookstatus']];?></td>
                                  <td align="right"><?php echo number_format($row['bookmoney']);?></td>
                                  <td>
                                  <a id="bookingdoc"  name="bookingdoc" class="btn btn-primary waves-effect waves-light"   value="" target="_blank" href="preemption.php?id=<?php echo $row["bookid"];?>">ใบจอง</a>                                
                                  </td>
                                  <td> 
                                  <?php 
                                  if($row['bookstatus']==3){
                                  ...
                                  ?> 
                                  <a id="<?php echo $row['bookid'];?>" 
                                   id="rent" class="btn btn-success waves-effect waves-light"   value="" href="save_rent.php?id=<?php echo $row["bookid"];?>">เช่า</a>
                                  <?php }?>
                                  </td>
                                  ...
                                  ?>                 
                                  <a id="<?php echo $row['bookid'];?>" 
                                  class="delbook btn btn-danger waves-effect waves-light" type="button" value="" href="#">ยกเลิกรายการจอง</a>                              
                                  <?php } ?>
                                  </td>
                                </tr>
                                <?php } ?>
